...
% %         legend_length = 0;
% %     end
% %     [mu_bounds_length,~]= size(mu_bounds);
% %     for i = 1:mu_bounds_length
% %         legend_plots{i+legend_length} = plot(nan,'Color',colors(i,:)) ;
% %         legend_labels{i+legend_length} = [num2str(mu_bounds(i,1)) ' to ' num2str(mu_bounds(i,2)) ' MeV/G'];
% %     end
% %     %dim_legend = ;
% %     [~, hobj, ~, ~] = legend([legend_plots{:}],legend_labels{:},'location','best','FontSize',fontsize);
